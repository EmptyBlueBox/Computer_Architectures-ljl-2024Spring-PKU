...
这里 `||00` 代表的是将扩展后的 `Imm16` 左移两位，然后拼接两个 0。而不是或的意思。这是因为在 MIPS 架构中，跳转地址是按字节寻址的，而每条指令的长度是 4 个字节，所以需要将立即数左移两位（即附加两个零位）来得到正确的跳转地址。
...
5. 存储器
    ...
    - 一个只读的指令存储器，地址和数据均为 32 位
    - 一个可读写的数据存储器，地址和数据均为 32 位
    - 注：这两个存储器实际对应了 CPU 中的指令和数据高速缓存（Cache），而不是内存。
...
-   选择 ALU 的第二个操作数：来自寄存器堆的 busB （`ALUSrc=0` 时，对应之前的情况，输入是 rt）或者扩展后的立即数（`ALUSrc=1` 时，对应当前情况）
-   选择 Rw 的写入目标：rd（`RegDst=1` 时，对应之前的情况） 或 rt（`RegDst=0` 时，对应当前情况）
...
-   选择扩展的输出类型，是零扩展立即数（`ExtOp=0`）还是符号扩展后的立即数（`ExtOp=1`）
-   选择写入寄存器堆的数据来源，是来 ALU 的输出（`MemtoReg=0`），还是自数据存储器 （`MemtoReg=1`）

#### 访存指令的需求（Store）

<img src="./6.单周期处理器.assets/CleanShot 2024-04-10 at 19.01.34@2x.png" alt="CleanShot 2024-04-10 at 19.01.34@2x" style="zoom:50%;" />

现有的设计具有一个问题：不知道什么时候储存
...
-   是否将 busB 的内容写入到数据存储器中，取决于控制信号 `MemWr`，如果为 1，则写入。否则不写入。
...
然后根据图中红色的控制信号，进行计算，计算出来 busW （表现为输出稳定）之后，在下一个时钟上升沿就会写入到 rd 中
...
除了分支指令，均有此步骤。而且每条指令都是在一个周期内完成的。
...
注意，Imm16 代表的是目标跳转指令和下一条指令之间的差值，而且会对其左移两位（即乘以 4），这是因为 MIPS 架构中，跳转地址是按字节寻址的，而每条指令的长度是 4 个字节（32 位），所以需要将立即数左移两位（即附加两个零位）来得到正确的跳转地址。
...
-   `ALUSrc`：算术逻辑单元（ALU）源选择信号，决定 ALU 的第二个操作数是来自寄存器还是立即数。
    -   `ALUSrc=0` 时，选择寄存器作为第二个操作数；
    -   `ALUSrc=1` 时，选择立即数作为第二个操作数。
...
根据不同的输入信号，ALU会执行不同的算术或逻辑操作。这些信号通常由控制单元根据指令的类型产生。
