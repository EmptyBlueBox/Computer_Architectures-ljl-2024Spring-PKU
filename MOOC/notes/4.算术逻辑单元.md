...
C 为高电平时，D 的输出可以通过第一个 D-latch 传入到第二个 D-latch 的输入 D，但是无法传到第二个 D-latch 的输出 Q。
...
优点：计算 $C_{i+1}$ 的延迟时间固定为 **三级门延迟** ，与加法器的位数无关
